...
const TrainingProgramCard: React.FC<TrainingProgramCardProps> = ({ program, onClick }) => {
  ...
  return (
    <button 
      ...
      className="relative w-full h-48 rounded-[2.5rem] overflow-hidden shadow-lg border border-white/5 group active:scale-[0.98] transition-all hover:border-primary/30"
    >
      <img 
        src={program.image} 
        alt={program.name} 
      ...
      <div className="relative z-10 p-6 flex flex-col justify-end h-full text-left">
        <h3 className="text-white text-xl font-black italic uppercase tracking-tight font-lexend leading-tight mb-2">
          {program.name}
        </h3>
        <p className="text-slate-400 text-xs font-bold leading-snug mb-4 line-clamp-2">
          {program.description}
        </p>
